...
/*   Updated: 2021/02/10 16:22:20 by hopark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	g->img.data[(int)pt.y * IMG_WIDTH + (int)pt.x] = color;
}
...
void			ft_draw_line(t_game *g, t_fvector v1, t_fvector v2, int color)
{
	...
	{
		g->img.data[(int)v1.y * IMG_WIDTH + (int)v1.x] = color;
		v1.x += dx;
		v1.y += dy;
	}
}
							...
							int size, unsigned int color)
{
	...
	while (i < size)
	{
		...
		while (j < size)
		{
			g->img.data[(size * pt.y + i) * IMG_WIDTH\
					+ size * pt.x + j] = color;
			j++;
		}
		i++;
