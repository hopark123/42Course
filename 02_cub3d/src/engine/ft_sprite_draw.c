...
/*   Updated: 2021/02/25 18:17:55 by hopark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_sprite_init(t_game *g, t_list *cur, t_sprites *spr)
{
	...
	spr->screenX = (g->size.x / 2) * (1 + spr->trans.x / spr->trans.y);
	spr->sprite.y = fabs((g->size.y / spr->trans.y));
	spr->sprite.x = fabs((g->size.x / spr->trans.y));
}
...
void			ft_sprite_draw(t_game *g, t_sprites *spr)
{
	...
	draw.end.x = ft_min(spr->screenX + spr->sprite.x / 2, g->size.x - 1);
	while (draw.start.x < draw.end.x)
	{
		draw.start.y = ft_max((g->size.y - spr->sprite.y) / 2, 0);
		draw.end.y = ft_min((g->size.y + spr->sprite.y) / 2, g->size.y - 1);
		...
		if (spr->trans.y > 0 && spr->trans.y < g_ZBUFFER[(int)draw.start.x])
			...
			{
				draw.step.y = draw.start.y * 256 - g->size.y * 128 + spr->sprite.y * 128;
