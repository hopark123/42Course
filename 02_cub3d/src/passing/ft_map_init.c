...
/*   Updated: 2021/02/25 17:22:20 by hopark           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_dfs(t_game *g, int i, int j, char **dfs)
{
	int			k;
	int			tx;
	int			ty;
...
int					ft_map_valid(t_game *g, int i, int j)
{
	...
	{	
		...
		dfs[k][g->map.size.x + 1] = 0;
		k++;
	}
	ft_dfs(g, i, j, dfs);
	if (ft_dfs_check(g, dfs) == -1)
		return (-1);
	return (0);
}
